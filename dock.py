...
class Dock(object):
    ...
    def __init__(self):
        ...
        scrollable_frame.bind("<Enter>", self.mousewheel_bound)
        scrollable_frame.bind("<Leave>", self.mousewheel_unbound)
        ...
        # Button creation
        minimize = tk.Button(self.titleFrame, text="\/", width=31, command=lambda: self.min_but(screen_width, screen_height, maximize, self.titleFrame, self.buttonFrame, self.folderFrame), bg="medium spring green", relief="flat")
        ...
        maximize = custom_button(self.root, text="/\\",command=lambda: self.max_but(w, h, x, y, maximize, self.titleFrame, self.buttonFrame, self.folderFrame), bg="medium spring green", relief="flat", font=("BOLD, 12"))
        maximize.menu.add_command(label="Color", command=partial(maximize.change_color, maximize))
        maximize.menu.add_command(label="Font Color", command=partial(maximize.font_color, maximize))
        maximize.menu.add_command(label="Unlock", command=partial(maximize.move_x_axis, maximize))
        maximize.bind("<Button-3>", maximize.color_popup)
        ...
        delNote = tk.Button(self.buttonFrame, text="Edit", width=5, command=lambda: self.set_editable(delNote), relief="flat")
    ...
    def min_but(self, scw, sch, maxb, tf, bf, ff):
        ...
        self.root.geometry('%dx%d+%d+%d' % (1920, 25, 0, (sch - 65)))
        self.root.overrideredirect(True)
        tf.pack_forget()
        bf.pack_forget()
        ff.pack_forget()
    ...
    def new_note(self, frame, index):   # Have to pass self.count as index or else self.notepads[self.count] does not accept input
        self.notepads.append(Notepad(self, self.count))
        self.notepads[index].closeOverride()
        self.notepadButton = custom_button(frame, text="New label #%d" % (index), width=37, command=lambda: [self.notepads[index].openOverride()], relief="flat")
        ...
        self.notepadButton.grid(column=0, columnspan=2, row=self.count, sticky=tk.N+tk.S+tk.E+tk.W, pady=(0, 10))
        ...
        if(self.editable == True):
            for note in self.button_list:
                if(note.selected == True):
                    ...
                    print(note.cget("text"))
                    print(self.notepadButton.cget("text"))
                    print("2")
                else:
                    print("pass")
    ...
    def first_selection(self, target1):
        ...
        while(target1.selected == True):
            ...
            self.root.update()
            self.root.after(600, target1.configure(bg="cyan2"))
            self.root.update()
            self.root.after(600, target1.configure(bg="SpringGreen1"))
            self.root.update()
...
class Notepad(Dock):
    ...
    def __init__(self, parent, position):
        ...
        # Creating label
        self.titleLabel = create_label(self.top, text="Title", bg="light blue", font=("Times 20 italic bold"))
        self.titleLabel.newParent = parent    # To set the main application as the parent of create_label
        self.titleLabel.row = self.position
    ...
    def openOverride(self):
        self.titleLabel.configure(bg=self.parent.button_list[self.position].cget("bg"))
        self.saveFile()
        self.top.deiconify()
...
class custom_button(tk.Button):
    ...
    def change_color(self, but):
        colors = askcolor(title="Background Color")
        self.bgcolor = colors[1]
        but.configure(bg=self.bgcolor)

    def font_color(self, but):
        colors = askcolor(title="Font Color")
        but.configure(fg=colors[1])

    def move_x_axis(self, but):
        but.locked = False
        but["state"] = "disabled"
        custom_button.make_draggable(but)
